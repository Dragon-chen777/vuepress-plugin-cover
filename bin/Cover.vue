...
export default {
  ...
  data() {
    return {
      ...
      randomContent: CONFIG.main[Math.random() * CONFIG.main.length | 0] // 随机内容
    }
  },
  computed: {
    textArr() {
    ...
    globalSty() {
      return {
        ...
        '--text-font-size': this.fontSize + CONFIG.pixels,
        '--text-font-weight': CONFIG.fontWeight,
        '--text-line-height': this.fontSize * 1.5 + CONFIG.pixels,
        '--text-writing-mode': CONFIG.writingMode,
  ...
  beforeMount() {
    ...
    if (CONFIG.isShowOnce && sessionStorage.getItem('isNoxoneCoverShowed')) return
  ...
  mounted() {
    if (CONFIG.isShowOnce && sessionStorage.getItem('isNoxoneCoverShowed')) return this.isShow = false
